...
public class OPGameController {
	...
	public void setBullet(OPGameFrame f, OPGraphicPanel p, Point point) {
		for(char c :stackTweets.get(mouseClickCount).toCharArray()){
			stackCharacters.add(this.myselfFactory.createCharacter(f.getWidth() - myself.getWidth(), f.getHeight()/2, (int)(point.getX()-f.getWidth()+myself.getWidth()), (int)(point.getY()-f.getHeight()/2), String.valueOf(c)));	
		}
		mouseClickCount++;
